...
object clusterTest {
    ...
    val tss : List[Double] = List(0.25,0.05,0.075,0.1,0.125,0.15,0.175,0.2,0.225,0.25)
  ...
  def prettyPrint(cn:Double):Unit = {
    val str = cn+" "+ecs(y)+" "+ecs(yLow)+" "+ecs(yHigh)+" "+ss(y)+" "+ss(yLow)+" "+ss(yHigh)+" "+qms(y)+" "+qms(yLow)+" "+qms(yHigh)+" "+ads(y)+" "+ads(yLow)+" "+ads(yHigh)+" "+acs(y)+" "+acs(yLow)+" "+acs(yHigh)+"\n"
    out.write(str)
    out.flush
    print(str)
  }
